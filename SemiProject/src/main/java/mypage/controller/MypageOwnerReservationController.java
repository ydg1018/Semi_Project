...
@WebServlet("/mypage/ownerReservation")
public class MypageOwnerReservationController extends HttpServlet {
...
private MypageService mypageService = new MypageServiceImpl();
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		System.out.println("/mypage/ownerReservation [GET]");

		//세션정보 객체
		HttpSession session = req.getSession();
		int ownerNo = (int) session.getAttribute("owner_no");

		/**-----------로그인한 일반회원정보 조회 OWNER -----------**/
		//서비스로 보낼 파라메터 데이터 셋팅
		Owner ownerparam = new Owner();
		ownerparam.setOwnerNo(ownerNo);
		...
		resparam.setOwnerNo(ownerNo);

		//로그인한 일반회원의 ownerReservation 정보 조회
		Reservation resData = mypageService.getOwnerReservation(resparam);
		System.out.println("로그인한 일반회원의 getOwnerReservation 정보 조회 data : " + resData);
		/**--------------------------------------------**/
		
		/**-----------병원회원정보 조회 HOS -----------**/
		//서비스로 보낼 파라메터 데이터 셋팅
		Hos param = new Hos();
		param.setHosCode(resData.getHosCode());
		...
		infoparam.setHos_code(resData.getHosCode());

		//로그인한 병원회원의 hosInfo 정보 조회
		HosInfo infoData = mypageService.getHosInfo(infoparam);
		System.out.println("병원회원의 HosInfo 정보 조회 data : " + infoData);
		/**--------------------------------------------**/
		...
		//조회결과를 MODEL값 전달
		req.setAttribute("data", data);				//병원회원 데이터
		req.setAttribute("resData", resData);		//예약 데이터
		req.setAttribute("infoData", infoData);		//예약병원 데이터
		req.setAttribute("ownerData", ownerData); 	//로그인 예약자 데이터
